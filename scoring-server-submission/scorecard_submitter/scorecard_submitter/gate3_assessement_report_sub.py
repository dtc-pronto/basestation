...
class AssessmentReportNode(Node):
        ...
        for robot in self.robots:
            if robot in UGV:
                ...
                self.get_logger().info(
                    f"Subscribed to /{robot}/triage_report/gate1 and /{robot}/triage_report/gate3"
                )
    ...
    def _load_ground_truth_database(self):
        ...
        if not os.path.exists(self.ground_truth_path):
            self.get_logger().error(f"Ground truth DB not found: {self.ground_truth_path}")
            return
    ...
    def _submit_field(self, robot: str, casualty_id: int, field_type: str, value: int, time_ago: float):
        response = trauma_report(
            type=field_type,
            value=int(value),
        ...
        if response.status_code != 200:
            self.get_logger().error(
                f"[{robot}] trauma_report failed for {field_type}: "
                f"{response.status_code} {response.text}"
            )
            return False

        self.get_logger().info(
            f"[{robot}] Submitted {field_type}={value} for casualty {casualty_id}"
        )
        return True
    ...
    def assessment_callback(self, msg: Gate3, robot: str):
        ...
        if lat is None or lon is None:
            self.get_logger().warn(
                f"[{robot}] Received Gate3 before Gate1 GPS; cannot resolve casualty ID"
            )
            return
        ...
        self.get_logger().info(
            f"[{robot}] Matched casualty {casualty_id} at {dist:.1f}m, submitting Gate3 block"
        )
        ...
        self.get_logger().info(
            f"[{robot}] Completed Gate3 submissions for casualty {casualty_id}. "
